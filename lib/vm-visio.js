...
function joinRoom(roomId, connection) {
    if (rooms.has(roomId)) {
        ...
        if (!room.isFull()) {
            const participantId = `participant-${++participant_index}`;
...
function publishToRoom(sdp, connection, endpoint) {
    ...
    connection.on("close",() => {
        ...
        participant.incomingStreams.forEach(elt => elt.stop());
...
function viewRemoteParticipant(remoteId, sdp, connection, endpoint) {
    ...
    connection.on("close",() => {
        if(transport) transport.stop();
        participant.incomingStreams.forEach(elt => elt.stop());
        participant.outgoingStreams.forEach(elt => elt.stop());
