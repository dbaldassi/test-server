...
const Native      = require("medooze-media-server/lib/Native");
...
function view(endpoint, connection, msg) {
    ...
    const fake = VideoCodecs.createFakeH264VideoEncoder({ fps: FPS, bitrate: constant_probing });

    //Create fake incomming video track
    const incomintTrack = fake.createIncomingStreamTrack("fake");
    // const player = MediaServer.createPlayer("www/quic-relay/moi.mp4");
    // player.play({repeat : true });

    // console.log(player);
    // console.log(player.getVideoTracks());
    // const incomintTrack = player.getVideoTracks()[0];
	...
	transport.on("targetbitrate", (bitrate) => {
	    ...
	    fake.setBitrate(FPS, encodingBitrate);
	    transport.setBandwidthProbing(encodingBitrate < probing);
	});
    } else {
	fake.setBitrate(FPS, constant_probing);
    }
    ...
    const source = incomintTrack.encodings.get("").source;
    incomintTrack.encodings.get("").depacketizer = new Native.RTPIncomingMediaStreamDepacketizer(source);
    incomintTrack.depacketizer = incomintTrack.encodings.get("").depacketizer;

    console.log(incomintTrack.depacketizer);
    
    const recorder = MediaServer.createRecorder("www/quic-relay/dumps/" + id + ".mp4");
    recorder.record(incomintTrack, { multitrack: false });
