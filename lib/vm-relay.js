...
//     session = streamer.createSession(video,{
// 	local  : {
// 	    port: 3456
// 	},
// 	// remote : {
...
//     // session.getIncomingStreamTrack().on("stopped", () => console.log("STOP"));
// }
...
let counter = 0;
var outgoingStream = [];
// var incomingStream = Cascade.incomingStream;
...
const STREAMER_PORT = 3456;

// var incomingStreamTracks = [];
...
module.exports = function(request, protocol, endpoint, ws_report)
{
	...
	if (msg.cmd === "publish") {
		...
		Cascade.incomingStream.push(is);
	    }

	    Cascade.media_info = offer.getMedia('video');
		...
		Cascade.incomingStream.forEach(elt => elt.stop());
		...
		let session = streamer.createSession(offer.getMedia('video'), {
		    local : { port: 3456 },
		});*/

		let session = Cascade.streamers[0];
		// session.getOutgoingStreamTrack().stop();
		...
		const track = os.getVideoTracks()[0];
		connection.transponder = track.attachTo(session.getIncomingStreamTrack());
	    ...
	    // incomingStreamTracks.push(session.getIncomingStreamTrack());
